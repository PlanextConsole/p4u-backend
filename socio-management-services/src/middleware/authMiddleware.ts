...
const ROLE_PERMISSION_MAP: Record<string, string[]> = {
  ADMIN: ['*'],
  CUSTOMER: ['social.feed.read', 'social.post.write', 'social.interact'],
  VENDOR: ['social.feed.read', 'social.post.write', 'social.interact'],
};
...
const getPermissions = (auth: any): string[] => {
  ...
  for (const s of String(auth?.scope || '').split(' ').map((x) => x.trim()).filter(Boolean)) {
    merged.add(s);
  }
