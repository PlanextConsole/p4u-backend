...
/** Permissions for marketplace shopping (user web checkout, cart, own orders). */
const SHOPPER_PERMISSIONS = [
  'order.read.self',
  'order.write.self',
  'cart.read.self',
  'cart.write.self',
...
const ROLE_PERMISSION_MAP: Record<string, string[]> = {
  ADMIN: ['*'],
  CUSTOMER: [...SHOPPER_PERMISSIONS],
  // Vendors can log in on user web and place orders; keep vendor-scoped order read too
  VENDOR: ['order.read.vendor', ...SHOPPER_PERMISSIONS],
};

/** Realm roles allowed on shopper-facing commerce routes (cart, checkout, bookings, reviews). */
export const SHOPPER_REALM_ROLES = ['ADMIN', 'CUSTOMER', 'VENDOR'] as const;
...
export const requireShopperRole = requireAnyRole([...SHOPPER_REALM_ROLES]);
